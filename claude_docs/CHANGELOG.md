...
- **Opt-in real-time streaming** via FluidAudio's EOU 120M model — partial text appears live in the overlay while the user speaks
- **Either/or model architecture:** streaming mode uses EOU 120M exclusively; batch mode uses TDT V3 (~600M params). The two models do not run simultaneously.
- **Audio processing:** EOU model processes 160ms audio chunks during active recording via `onAudioBuffer` callback in `AudioRecorder`
- **Partial overlay state:** new `.streamingPartial` overlay state shows ghost text during recording; finalises on release
- **`StreamingTranscriber` protocol** — new swappable interface for streaming backends (parallel to existing `Transcriber` protocol)
- **`StreamingEouAsrManager`** — manages EOU model lifecycle, chunk delivery, and partial result callbacks
- **New config key:** `streaming_enabled` (Bool, default `false`) — persisted in `~/.textecho_config`
- **Enable via Settings → Streaming (Beta)** — download EOU model (~120M params), then toggle on
- Full batch TDT path is unaffected when streaming is disabled (default behaviour preserved)

### Silence Skip Removal (PR #40)

- Removed RMS silence gate from both `ParakeetTranscriber` and `WhisperKitTranscriber`
- Quiet or whispered speech is no longer silently discarded before reaching the transcription model
- The transcription model itself handles low-energy audio; pre-filter was causing missed dictations
- `silence_threshold` config key retained for the auto-stop silence detection (separate concern — this still works)
